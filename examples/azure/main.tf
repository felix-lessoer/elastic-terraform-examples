...
resource "terraform_data" "seed_azure_insight_indices" {
  count = length(module.observability) > 0 ? 1 : 0

  triggers_replace = [
    filesha256("${path.module}/../../modules/cockpit-dashboard/scripts/seed_azure_insight_indices.py"),
    filesha256("${path.module}/../../modules/cockpit-dashboard/scripts/insight_fabric_common.py"),
    filesha256("${path.module}/../../modules/cockpit-dashboard/cockpit-azure.ndjson"),
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      set -euo pipefail
      python3 "${path.module}/../../modules/cockpit-dashboard/scripts/seed_azure_insight_indices.py" \
        --obs-es "$OBS_ES" \
        --sec-es "$SEC_ES" \
        --obs-user "$OBS_USER" \
        --sec-user "$SEC_USER" \
      ...
      python3 "${path.module}/../../modules/cockpit-dashboard/scripts/generate_azure_recommendations.py" \
        --es-url "$OBS_ES" \
        --user "$OBS_USER" \
